...
def build_suggested_scenarios(bayes_read_df):
    ...
    This is meant to guide the manual scenario selections in the lineup builder.
    It does not lock anything by itself.
    """
    ...
    for game_info, g in bayes_read_df.groupby("game_info", dropna=False):
        ...
        ordered = g.sort_values("bayes_win_pct", ascending=False)
        ...
        # For lineup-building, choose the winner path from the winning team's 2-0 / 2-1 probabilities.
        if p_2_0 >= p_2_1:
            winner_result = "2-0"
            loser_result = "0-2"
        else:
        ...
        if sample_type == "League Prior Only" or sample < 5:
            confidence = "Low"
        elif gap >= 15 and sample >= 8:
            confidence = "High"
        elif gap >= 7 and sample >= 5:
            confidence = "Medium"
        else:
            confidence = "Low"

        if gap < 5:
            scenario_note = "Close matchup. Consider building alternate scenarios."
        elif winner_result == "2-0":
            scenario_note = f"{winner_team} sweep lean. Best fit for heavier {winner_team} stacks."
        else:
            scenario_note = f"{winner_team} win lean, but extended series. Opponent one-offs remain viable."
